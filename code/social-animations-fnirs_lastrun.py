...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/mizwally/Desktop/SCANS-social_animation_fNIRS/code/social-animations-fnirs_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Intro = visual.TextStim(win=win, name='Intro',
        text='You will now watch short clips of shapes and decide if they are having a social or non-social interaction.\n\nWhen the clip is finished, you will be prompted to make a decision. You will have 3 seconds to make a choice.\n\nFor a social interaction, press the left arrow key. For no interaction, press the right arrow key. If you are not sure, press the down arrow key. \n\n',
        font='Open Sans',
    ...
    unsure = visual.TextStim(win=win, name='unsure',
        ...
        languageStyle='LTR',
        depth=-1.0);
    non_social = visual.TextStim(win=win, name='non_social',
        text='Non-Social',
        font='Arial',
        pos=(0.5, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    for thisRun in Runs:
        ...
        for thisTrial in Trials:
            ...
            resp_phase = data.Routine(
                name='resp_phase',
                components=[social, unsure, non_social, left, down, right, resp_key],
            )
            ...
            while continueRoutine and routineTimer.getTime() < 3.0:
                ...
                if social.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > social.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                if unsure.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > unsure.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                # if non_social is starting this frame...
                if non_social.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    non_social.frameNStart = frameN  # exact frame index
                    non_social.tStart = t  # local t and not account for scr refresh
                    non_social.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(non_social, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'non_social.started')
                    # update status
                    non_social.status = STARTED
                    non_social.setAutoDraw(True)
                
                # if non_social is active this frame...
                if non_social.status == STARTED:
                    # update params
                    pass
                
                # if non_social is stopping this frame...
                if non_social.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > non_social.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
                        non_social.tStop = t  # not accounting for scr refresh
                        non_social.tStopRefresh = tThisFlipGlobal  # on global time
                        non_social.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'non_social.stopped')
                        # update status
                        non_social.status = FINISHED
                        non_social.setAutoDraw(False)
                ...
                if down.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > down.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
    ...
    end_task = data.Routine(
        name='end_task',
        components=[end_sound, end_task_text],
    )
